...
def run_comprehensive_tests():
    ...
    test_script = os.path.join("tests", "test_comprehensive.py")
    if os.path.exists(test_script):
        ...
        print(f"[WARN] {test_script} not found, skipping comprehensive tests")
...
if sys.platform == "win32":
    asyncio.set_event_loop_policy(asyncio.WindowsProactorEventLoopPolicy())
import uvicorn
uvicorn.run("jobsearch_agent.api.app:app", host="{host}", port={port}, log_level="{'debug' if debug else 'info'}")
"""
        ])
