...
python -m jobsearch_agent.scraper.buggmenot glassdoor.com

# With output file
python -m jobsearch_agent.scraper.buggmenot nytimes.com --output credentials.json

# With browser and anonymization options
python -m jobsearch_agent.scraper.buggmenot wsj.com \
  --browser firefox \
...
```python
from jobsearch_agent.scraper.buggmenot import BugMeNotScraper
import asyncio
...
```python
from jobsearch_agent.scraper.buggmenot import get_credentials, get_multiple_credentials
import asyncio
...
```bash
# With proxy
python -m jobsearch_agent.scraper.buggmenot glassdoor.com --proxy http://proxy:8080

# Disable anonymization
python -m jobsearch_agent.scraper.buggmenot nytimes.com --no-anonymize

# Firefox with SOCKS5 proxy
python -m jobsearch_agent.scraper.buggmenot wsj.com --browser firefox --proxy socks5://proxy:1080
```
...
python -m jobsearch_agent.scraper.buggmenot nytimes.com \
...
python -m jobsearch_agent.scraper.buggmenot wsj.com --output /dev/null

# With custom timeout
python -m jobsearch_agent.scraper.buggmenot economist.com --timeout 60
```
